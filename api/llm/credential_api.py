...
from schemas.llm import LLMCredentialResponse, LLMCredentialCreate, LLMCredentialDetail, LLMCredentialUpdate
from services.llm.credential_service import LLMCredentialService


@credential_router.get("/page_list", response_model=PageResponse[LLMCredentialResponse],
                       description="分页获取凭据")
async def get_credential_page_list(
        ...
        credential_service: LLMCredentialService = Depends(get_credential_service)
):
    """
    凭据分页查询
    :param request:
...
@credential_router.get("/list", response_model=Result[List[LLMCredentialResponse]],
...
@credential_router.get("/detail", response_model=Result[LLMCredentialDetail], description="获取凭据详情")
...
@credential_router.post("/create", response_model=Result[LLMCredentialResponse], description="创建凭据")
async def create_credential(
        request: Request,
        obj_in: LLMCredentialCreate,
        credential_service: LLMCredentialService = Depends(get_credential_service)
):
    """
    创建凭据
...
@credential_router.put("/update", response_model=Result[LLMCredentialResponse], description="更新凭据")
